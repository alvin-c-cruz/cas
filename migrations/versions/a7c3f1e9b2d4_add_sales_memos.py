"""add sales_memos and sales_memo_items (Credit/Debit memo)

Revision ID: a7c3f1e9b2d4
Revises: 29500ade76f8
Create Date: 2026-07-10 09:00:00.000000

Two new tables only -- no changes to any existing table. Real inline FKs are safe here
(fresh create_table, no metadata cycle); the batch-add-column FK trap only applies to
op.batch_alter_table(...).add_column on existing tables.
"""
...
def upgrade():
    op.create_table('sales_memos',
        ...
        sa.Column('sales_invoice_id', sa.Integer(), sa.ForeignKey('sales_invoices.id'), nullable=False),
        sa.Column('original_invoice_number', sa.String(length=50), nullable=False),
        ...
        sa.Column('destination', sa.String(length=20), nullable=False, server_default='ar'),
    ...
    with op.batch_alter_table('sales_memos', schema=None) as b:
        b.create_index('ix_sales_memos_memo_number', ['memo_number'], unique=True)
        b.create_index('ix_sales_memos_memo_type', ['memo_type'])
        b.create_index('ix_sales_memos_branch_id', ['branch_id'])
        b.create_index('ix_sales_memos_memo_date', ['memo_date'])
        b.create_index('ix_sales_memos_sales_invoice_id', ['sales_invoice_id'])
        b.create_index('ix_sales_memos_customer_id', ['customer_id'])
        b.create_index('ix_sales_memos_status', ['status'])
        b.create_index('ix_sales_memos_salesperson_id', ['salesperson_id'])

    op.create_table('sales_memo_items',
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('sales_memo_id', sa.Integer(), sa.ForeignKey('sales_memos.id'), nullable=False),
        sa.Column('line_number', sa.Integer(), nullable=False),
        sa.Column('sales_invoice_item_id', sa.Integer(), sa.ForeignKey('sales_invoice_items.id'), nullable=False),
    ...
    with op.batch_alter_table('sales_memo_items', schema=None) as b:
        b.create_index('ix_sales_memo_items_sales_memo_id', ['sales_memo_id'])
        b.create_index('ix_sales_memo_items_sales_invoice_item_id', ['sales_invoice_item_id'])


def downgrade():
    op.drop_table('sales_memo_items')
    op.drop_table('sales_memos')
