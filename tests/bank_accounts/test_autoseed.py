...
def _posted_je_on(db_session, account, branch, entry_number, debit=True):
    """A minimal posted JournalEntry with one line on `account` -- the seeder reads
    JournalEntry/JournalEntryLine directly, so the test builds those tables directly
    ...
    je = JournalEntry(entry_number=entry_number, entry_date=date(2026, 7, 1),
                      ...
                      branch_id=branch.id, status='posted',
