...
def test_cdv_resolves_ap_from_settings(db_session):
    _acct(db_session, '2110', 'AP - Trade')
...
def test_cdv_unassigned_wht_payable_optional_none(db_session):
    from app.posting.control_accounts import get_control_account
    assert get_control_account('wht_payable', required=False) is None
