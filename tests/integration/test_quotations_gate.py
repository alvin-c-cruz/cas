...
def test_quotations_list_blocked_when_module_off(client, db_session, admin_user, main_branch):
    ...
    resp = client.get('/quotations', follow_redirects=False)
    assert resp.status_code == 404          # gated off by default


def test_quotations_registry_entry_is_optional_so_gated_per_user(db_session):
    from app.users.module_access import MODULE_REGISTRY, all_permission_keys
    e = next(m for m in MODULE_REGISTRY if m['key'] == 'quotations')
    assert e['optional'] is True and e['per_user'] is True
    assert e['default_enabled'] is False and e['depends_on'] == ['sales_orders']
    assert 'quotations' in all_permission_keys()   # per_user keeps it in the grid


def test_quotations_list_ok_when_enabled(client, db_session, admin_user, main_branch):
    _enable(db_session, 'quotations')
    _login(client, admin_user)
    with client.session_transaction() as sess:
        sess['selected_branch_id'] = main_branch.id
