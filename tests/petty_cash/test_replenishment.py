...
def _setup(db_session, main_branch, cash_account, revenue_account, staff_user):
    ...
    # account below) -- if the bank leg shared an account_id with the expense
    # line, the JE would legitimately post two separate lines (one debit, one
    # credit) against the SAME account, which a naive {account_id: (dr, cr)}
    # assertion dict can't represent. Keeping them distinct here matches the
    # real-world shape (a funding bank account is never also an expense account).
    funding_gl = Account(code='10250', name='Cash - Petty Cash Funding', account_type='Asset',
                         normal_balance='Debit', is_active=True)
    db_session.add(funding_gl); db_session.commit()
    bank_ba = BankAccount(branch_id=main_branch.id, code='BA-REP', name='Funding',
                          account_id=funding_gl.id, account_type='checking', opening_balance=0)
...
def test_replenishment_je_groups_by_expense_account(db_session, main_branch, cash_account,
    ...
    assert lines[bank_ba.account_id] == (Decimal('0.00'), Decimal('1500.00'))
