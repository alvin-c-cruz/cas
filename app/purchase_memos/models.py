...
class PurchaseMemo(db.Model):
    ...
    __table_args__ = (
        ...
        db.Index('uq_purchase_memos_type_number', 'memo_type', 'memo_number', unique=True),
    )
...
def generate_purchase_memo_number(memo_type, branch_id=None):
    ...
    'credit') keeps its own independent sequence -- filtered by the memo_type
    COLUMN, not a prefix string, so the fix doesn't merge Vendor Debit Memo and
    Vendor Credit Memo numbering into one shared pool. Each memo gets the next
    number after the highest existing purely-numeric memo_number of that type --
    this deliberately includes legacy-migrated literal numbers, not just
