...
> **✅ RECONCILED 2026-06-21 — COA REBUILD COMPLETE.** All rows ticked to match
> reality: `instance/ric.db` holds **360 accounts** (20 top-level groups + 341
> postable leaves), and every code in this worklist was confirmed present in the
> DB. The checkboxes had simply never been ticked during the manual rebuild —
> this is a documentation reconciliation, not new data. Discrepancies noted:
> - **`11703`** exists in `ric.db` but has no row below (created during rebuild,
>   beyond the original snapshot) — intentionally left out of the table; recorded here.
> - **`12000`** appears twice in the table below (a group header + a leaf row) —
>   an authoring duplicate, harmless; the single account exists once in the DB.
