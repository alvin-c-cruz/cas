...
with sync_playwright() as pw:
    ...
    rows = q(f"SELECT cdv_number, notes FROM cash_disbursement_vouchers WHERE notes LIKE '{TAG}-%' ORDER BY id")
    ...
    if committed < N:
        print(f"\n  >>> CONFIRMS the pre-check ({N - committed} of {N} lost) does NOT close the race in CD either.")
