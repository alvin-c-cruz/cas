...
- **Tabbed pages reuse the shared `cas-ui` `.tabs` component** (`app/static/js/cas-ui.js` delegated click handler + `.tabs`/`.tab`/`.tab-content` in `style.css`) — emit `.tab[data-tab-group="G"][data-tab="X"]` buttons (**`type="button"`** — they often sit inside a `<form>`, and a bare button submits it) plus `.tab-content[data-tab-panel="G"][id="G-X"]` panels; add **nothing** to JS/CSS. Panels may sit **outside** a `<form>` (the handler wires by `data-*`/`id`, not DOM containment) so a page can keep its single form on one tab while other tabs hold their own separate forms. Company Settings (`company_settings/form.html`) is the first consumer. See memory `reference-cas-ui-tabs-component`.
...
- **In-form submit buttons say "Save" / "Update"; list/launch buttons keep their own verb ("Enter" for transaction documents, "Create" for master data).** The in-form submit button on documents that post to the books (purchase bills, sales invoices, receipts, journal entries) reads **"Save"** (create) / **"Update"** (edit) — plain, no document name. Reference/master records (vendors, customers, accounts, branches, users, VAT/WHT codes, products, UOM, product categories, work centers, asset categories, bank accounts) follow the **same Save/Update verb pair** but **keep the record noun** — e.g. "Save Customer" / "Update Customer", "Save Parent Account" / "Update Parent Account" (changed 2026-07-20 from the older "Create X" convention — owner directive, applied app-wide). List-page launch buttons are a separate UI element (a link that opens the form, not a submit) and are unaffected by this: transaction documents keep **"Enter …"** (e.g. "+ Enter APV", "+ Enter CDV", page titles may still read "Enter …"), master data keeps **"+ Create …"** (e.g. "+ Create Customer").
...
- **Loosening a route-level guard does not loosen its template's parallel guard — check both.**
  `delete_approved_email` had its `is_used` block removed (to let an admin delete a *used* row),
  but `approved_emails_list.html` kept `{% if not email.is_used %}` around the Delete button —
  backend-complete with zero reachable UI path, invisible to every pytest test because they POST
  the delete URL directly instead of clicking a rendered button. Whenever a task removes/loosens a
  view-level authorization or visibility check, grep the same feature's template(s) for a parallel
  `{% if %}` guard on the control that reaches that route. See memory `feedback-premerge-uitest-gate`.
